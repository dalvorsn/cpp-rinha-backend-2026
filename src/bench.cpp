...
static std::vector<HistBucket> make_hist(const IndexStats& idx,
                                         int nbuckets = 20) {
...
static std::vector<Entry> load_entries(const char* path) {
  ...
  if (!ea) {
    fprintf(stderr, "No 'entries' key in %s\n", path);
    return {};
  }
  ...
  while (p < end) {
    ...
    const char* rk =
        (const char*)memmem(obj, (size_t)(q - obj), "\"request\"", 9);
    if (!rk) {
      p = q + 1;
      continue;
    }
    ...
    const char* ek = (const char*)memmem(obj, (size_t)(q - obj),
                                         "\"expected_approved\"", 19);
    if (!ek) {
      p = q + 1;
      continue;
    }
...
static void write_markdown(const char* path, const CpuInfo& cpu,
                           const IndexStats& idx, size_t n_entries, int n_fraud,
                           int n_edge, int n_borderline,
  ...
  fprintf(md,
          "| **CPU limit** | 0.37 cores (≈ Core i5-4260U @ 1.4 GHz "
          "single-thread) |\n");
  fprintf(
      md,
  ...
  fprintf(md, "| **Borderline (detected)** | %d (%.1f%%) |\n\n", n_borderline,
          n_entries ? n_borderline * 100.0 / n_entries : 0.0);
  ...
  for (int i = 0; i < (int)results.size(); ++i)
    if (results[i].fp == 0 && results[i].fn == 0 &&
        results[i].p99_us < best_p99) {
  ...
  fprintf(md, "## Results\n\n");
  fprintf(md,
          "> `approved = fraud_neighbors / 5 < 0.6` — threshold is fixed by "
          "the server.\n\n");
  fprintf(md,
          "| NP | NP.BORDER | R.MIN | R.MAX | avg (µs) | p50 (µs) | p99 (µs) | "
          "max (µs) | TP | TN | FP | FN | FP%% | FN%% |\n");
  fprintf(md, "|---|---|---|---|---|---|---|---|---|---|---|---|---|---|\n");
...
int main(int argc, char* argv[]) {
  ...
  fprintf(stderr, "CPU: %s (%d cores / %d threads", cpu.model.c_str(),
  ...
  for (const auto& e : entries)
    if (!e.expected_approved) ++n_fraud;
  ...
  for (size_t i = 0; i < entries.size(); ++i) {
    ...
    if (norm.normalize_raw(e.request_json.data(), (int)e.request_json.size(),
                           vecs[i].data()))
      ++norm_ok;
    else
      vecs[i].fill(0);
  ...
  write_markdown(md_path, cpu, idx, entries.size(), n_fraud, 645, n_borderline,
                 all_results);
