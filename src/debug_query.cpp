...
int main(int argc, char* argv[]) {
    ...
    for (uint32_t bi = blk_start; bi < blk_end; bi++) {
      ...
      for (int lane = 0; lane < n_valid; lane++) {
        ...
        if (d < worst) {
          ...
          int wi = 0;
          for (int i = 1; i < TOP; i++)
            if (top[i].dist > top[wi].dist) wi = i;
          top[wi] = {d, vec_idx, lbl[lane]};
          worst = 0;
          for (int i = 0; i < TOP; i++)
            if (top[i].dist > worst) worst = top[i].dist;
        }
        vec_idx++;
      }
      pos += n_valid;
  ...
  for (int i = 0; i < IVF_DIMS; i++)
    printf("%d%s", QUERY[i], i < IVF_DIMS - 1 ? ", " : "");
  printf("]\n\n");
  ...
  for (int i = 0; i < TOP; i++) {
    ...
    printf(
        "  [%d] dist_int16²=%-12.0f  dist_float=%.6f  label=%s  vec=%u  %s\n",
        i + 1, top[i].dist, sqrtf(top[i].dist) / 10000.0f,
        top[i].label ? "FRAUD" : "legit", top[i].idx, tag);
    if (i < 5 && top[i].label) fraud_count++;
  }
  printf("\nfinal fraud_count=%d → %s\n", fraud_count,
         fraud_count >= 3 ? "approved:false (fraud)" : "approved:true (legit)");
