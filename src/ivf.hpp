...
class IVF {
 ...
 private:
    ...
    for (uint32_t g = 0; g < n_groups; g++) {
      ...
      for (int p = 0; p < IVF_PAIRS; p++) {
        ...
        acc = _mm256_add_ps(acc,
                            _mm256_cvtepi32_ps(_mm256_madd_epi16(diff, diff)));
      }
      float vals[8];
  ...
  __attribute__((always_inline)) inline void update_top5(float dist,
                                                         uint8_t label,
                                                         float* top_dists,
                                                         uint8_t* top_labels,
                                                         float& max_top) const {
    ...
    if (dist < top_dists[0]) {
      ...
      top_labels[4] = top_labels[3];
      top_dists[3] = top_dists[2];
      top_labels[3] = top_labels[2];
  ...
  void scan_cluster(uint32_t c, const int16_t* q, float* top_dists,
                    uint8_t* top_labels, float& max_top) const {
...
#define SPAIR(p, acc)                                                        \
  {                                                                          \
    ...
    for (uint32_t bi = blk_start; bi < blk_end; bi++) {
      ...
      if (_mm256_movemask_ps(_mm256_cmp_ps(_mm256_add_ps(acc0, acc1),
                                           _mm256_set1_ps(max_top),
                                           _CMP_GE_OQ)) == 0xFF)
        continue;
...
#undef SPAIR
  ...
  float bbox_lower_bound(uint32_t c, const int16_t* q) const {
    ...
    for (int d = 8; d < IVF_DIMS; d++) {
      float v = (float)q[d];
      float g = v<(float)mn[d] ? (float)mn[d] - v : v>(float) mx[d]
                    ? v - (float)mx[d]
                    : 0.0f;
      lb += g * g;
    }
