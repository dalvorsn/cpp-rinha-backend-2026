...
class KNN {
 public:
    ...
    int get_fraud_count(const float* target_f) const {
        ...
        search(0, target_f, tq, top_dists, top_frauds, max_top);
 ...
 private:
    inline void update_top5(float dist, bool fraud,
                             ...
                             float& max_top) const __attribute__((always_inline)) {
...
#define KDIM(d, vt, acc) { \
    ...
    acc = _mm256_fmadd_ps(df, df, acc); \
        ...
        for (int bi = 0; bi < leaf_count; bi++) {
            ...
            KDIM(6,  vt6,  acc0); KDIM(7,  vt7,  acc1);
            ...
            for (int j = 0; j < b.n_valid; j++)
                update_top5(dists[j], (bool)b.is_fraud[j],
                            top_dists, top_frauds, max_top);
        }
#undef KDIM
    }

    void search(int idx, const float* query, const uint16_t* tq,
                float* top_dists, bool* top_frauds, float& max_top) const {
        const BallNode& n = nodes[idx];

        // Lower bound on distance from query to any point in this ball
        float dsq = 0;
        for (int d = 0; d < 14; d++) {
        ...
        float closest  = dist - n.radius;
        if (closest > 0.0f && closest * closest >= max_top) return;
        ...
        // Visit the child whose center is closer to query first
        // (reduces max_top sooner → more aggressive pruning in second child)
        ...
        if (ld <= rd) {
            search(n.left,  query, tq, top_dists, top_frauds, max_top);
            search(n.right, query, tq, top_dists, top_frauds, max_top);
        } else {
            search(n.right, query, tq, top_dists, top_frauds, max_top);
            search(n.left,  query, tq, top_dists, top_frauds, max_top);
        }
