...
static int recv_fd(int sock) {
    ...
    if (r == 0) { fprintf(stderr, "[ERR] ctrl closed by LB\n"); return -1; }
...
static void handle_client_read(int fd) {
    ...
    if (!c) { drop_conn(fd); return; }

    int room = BUF_CAP - c->buf_len;
    if (room <= 0) { drop_conn(fd); return; }
    ...
    if (c->headers_size == 0) {
        ...
        DLOG("fd=%d hdr=%d fraud=%d clen=%d\n",
             fd, c->headers_size, c->is_fraud, c->content_len);
    }
    ...
    if (c->is_fraud) {
        ...
        if (!ok) { resp = ERR_RESP; resp_len = ERR_RESP_LEN; }
    }
...
static void handle_client_write(int fd) {
    Conn* c = (fd >= 0 && fd < MAX_FDS) ? conns[fd] : nullptr;
    if (!c || !c->send_ptr) { drop_conn(fd); return; }

    ssize_t n = send(fd, c->send_ptr + c->send_off,
...
int main(int argc, char* argv[]) {
    ...
    if (listen_ctrl < 0) { perror("ctrl socket"); return 1; }
    ...
    while (true) {
        ...
        for (int i = 0; i < nfds; i++) {
            ...
            if (fd == ctrl_fd) { accept_from_lb(); continue; }

            if (e & (EPOLLHUP | EPOLLERR)) { drop_conn(fd); continue; }
