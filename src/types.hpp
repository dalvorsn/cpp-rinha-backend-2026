...
inline IndexLayout layout_for(uint32_t k, uint32_t total_blocks) {
    ...
    l.centroids = off; off += size_t(k) * IVF_DIMS * sizeof(int16_t);
    l.bbox_min  = off; off += size_t(k) * IVF_DIMS * sizeof(int16_t);
    l.bbox_max  = off; off += size_t(k) * IVF_DIMS * sizeof(int16_t);
    off = ivf_align(off, alignof(uint32_t));
    l.offsets   = off; off += size_t(k + 1) * sizeof(uint32_t);
    l.counts    = off; off += size_t(k) * sizeof(uint32_t);
    l.labels    = off; off += size_t(total_blocks) * IVF_BLOCK;
    off = ivf_align(off, alignof(int16_t));
    l.blocks    = off; off += size_t(total_blocks) * IVF_DIMS * IVF_BLOCK * sizeof(int16_t);
    l.total     = off;
    return l;
}
